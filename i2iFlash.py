# --- START OF FILE sdxl_instruct_pix2pix.py (Fixed Image Size Bug) ---
import torch
import random
import numpy as np
...
class i2iFlash:
    ...
    def edit_image(self, model_file, image, instruction, guidance_scale, image_guidance_scale, num_inference_steps, seed, width, height):
        ...
        with torch.no_grad():
            # === การแก้ไขที่สำคัญ: เพิ่ม width และ height เข้าไปใน pipe() call ===
            result_image = self.pipe(
                prompt=instruction, 
                image=pil_image, 
                width=width,                  # <--- บอกขนาดที่ต้องการ
                height=height,                # <--- บอกขนาดที่ต้องการ
